...
public class transactionMathType
{
    public string type;
...
public sealed class MathPlugin
{
    ...
    [KernelFunction, Description("Add two numbers, also used to for the transactions of  the customer financial data")]
    ...
    [KernelFunction, Description("Subtract two numbers, also used to for the transactions of  the customer financial data")]
    ...
    [KernelFunction, Description("SUm total of all the transactions of  the customer financial data")]
    public static double TransactionSummary(
       [Description("The transaction JSON")] string transactionData 
       
   )
    {

        Transaction[] arrtransactionData = JsonConvert.DeserializeObject<Transaction[]>(transactionData);

        double total=0;

        foreach( var item in arrtransactionData )
        {
            if(item.type=="Credit")
                total += item.amount;
    ...
    [KernelFunction, Description("SUm total of all credit transactions of  the customer financial data")]
    public static double CreditTransactionSummary(
      [Description("The transaction JSON")] string transactionData
    ...
    [KernelFunction, Description("SUm total of all Debit transactions of  the customer financial data")]
    public static double DebitTransactionSummary(
  [Description("The transaction JSON")] string transactionData
